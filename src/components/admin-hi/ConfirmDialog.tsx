  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="confirm-dialog-title">
      ...
      <div className="relative bg-card border border-gold/20 p-8 max-w-md w-full mx-4 anim-tilt-in">
        <button onClick={onCancel} className="absolute top-4 right-4 p-1 hover:bg-gold/10" aria-label="Fermer">
          <X className="w-4 h-4 text-gold/60" />
        </button>
        <h3 id="confirm-dialog-title" className="font-display text-xl mb-3">{title}</h3>
