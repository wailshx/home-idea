-- Drop old unique constraint that only considers booking_id
DROP INDEX IF EXISTS unique_active_dispute_per_booking;

-- Create new constraint: one active dispute per (booking, user)
CREATE UNIQUE INDEX unique_active_dispute_per_booking_user 
ON public.disputes(booking_id, initiated_by_user_id) 
WHERE status IN ('open', 'in_progress', 'escalated');

-- Update the dispute creation function to check by booking AND user
...
BEGIN
  ...
  -- Find existing support thread for THIS USER and this booking
