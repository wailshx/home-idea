-- Fix double-subtraction of dispute refunds in actual_net_earnings
-- Option A: Remove dispute_refunds from the calculation since net_earnings already reflects them
...
BEGIN
  ...
  final_rows AS (
    SELECT 
      ...
      CASE 
        WHEN DATE_PART('day', DATE_TRUNC('month', lm.booking_month) + INTERVAL '1 month' - INTERVAL '1 day')::NUMERIC > 0
        THEN ROUND(
      ...
      -- FIXED: Remove dispute_refunds subtraction since net_earnings already reflects them
      ROUND(
        (COALESCE(ps.total_net, 0) + 
         COALESCE(ci.cancellation_income, 0) + 
         COALESCE(di.income_amount, 0))::NUMERIC, 
        2
