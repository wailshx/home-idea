-- Update host_search_payouts to return NULL instead of 0 for optional numeric fields
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      WHEN p.transaction_type = 'debt_collection' THEN 'debt_collection'
    ...
    CASE WHEN b.status IN ('cancelled_guest'::booking_status, 'cancelled_host'::booking_status) 
      THEN NULLIF(ABS(p.amount), 0) ELSE NULL END AS refund_amount,
    CASE WHEN b.status IN ('cancelled_guest'::booking_status, 'cancelled_host'::booking_status) 
...
-- Update payouts table to set default NULL for optional numeric fields
ALTER TABLE public.payouts 
  ALTER COLUMN debt_applied_amount SET DEFAULT NULL,
  ALTER COLUMN original_amount SET DEFAULT NULL;