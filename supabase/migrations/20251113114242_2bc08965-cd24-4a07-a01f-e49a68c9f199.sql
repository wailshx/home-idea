...
-- Recreate function with fixed ROUND type casting
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      THEN ROUND((bs.cancelled_count::numeric / (bs.completed_count + bs.cancelled_count)::numeric * 100)::numeric, 2)
      ELSE 0
    ...
    CASE 
      ...
      THEN ROUND((COALESCE(bs.total_nights, 0)::numeric / DATE_PART('days', DATE_TRUNC('month', lm.booking_month) + INTERVAL '1 month' - DATE_TRUNC('month', lm.booking_month))::numeric * 100)::numeric, 2)
      ELSE 0
