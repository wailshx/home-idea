-- Drop all possible variants of get_host_earnings_report function
...
DROP FUNCTION IF EXISTS public.get_host_earnings_report(uuid, text, date, date, numeric, numeric);
DROP FUNCTION IF EXISTS public.get_host_earnings_report(uuid);
DROP FUNCTION IF EXISTS public.get_host_earnings_report;

-- Create the single version with the exact SQL from migration 20251113124608
...
BEGIN
  ...
  SELECT 
    ...
    fr.nights_booked,
    fr.completed_count,
    fr.cancel_percentage,
    fr.occupancy_percentage,
    fr.average_nightly_rate,
    fr.gross_earnings,
    fr.platform_fees,
    fr.net_earnings,
    fr.last_payout_date,
    fr.cancellation_income,
    fr.dispute_refunds,
    fr.dispute_income,
    fr.actual_net_earnings
  FROM final_rows fr
  WHERE 
    (p_min_gross IS NULL OR fr.gross_earnings >= p_min_gross)
  ...
  ORDER BY
    ...
    CASE WHEN p_sort_by = 'listing_title' AND p_sort_order = 'asc' THEN fr.listing_title END ASC,
    CASE WHEN p_sort_by = 'listing_title' AND p_sort_order = 'desc' THEN fr.listing_title END DESC,
    ...
    CASE WHEN p_sort_by = 'net_earnings' AND p_sort_order = 'desc' THEN fr.actual_net_earnings END DESC,
    CASE WHEN p_sort_by = 'net_earnings' AND p_sort_order = 'asc' THEN fr.actual_net_earnings END ASC,
    fr.month_date DESC;
END;
$function$;