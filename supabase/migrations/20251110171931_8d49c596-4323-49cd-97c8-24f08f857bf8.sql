-- Fix admin_resolve_dispute to accept "on_hold" decision and check for success: false in response
...
BEGIN
  ...
  IF p_is_submit THEN
    -- Validate decision (accept both "on_hold" and "hold")
    IF p_admin_decision NOT IN ('approved', 'declined', 'on_hold', 'hold') THEN
      RAISE EXCEPTION 'Invalid decision: must be approved, declined, on_hold, or hold';
    END IF;
    ...
    ELSE
      -- Hold/On Hold status - just update status to in_progress
      UPDATE public.disputes
      SET
        status = 'in_progress',
        admin_decision = p_admin_decision,
  ...
  ELSE
    -- Just saving draft (not submitting)
    UPDATE public.disputes
    SET
      admin_decision = p_admin_decision,
      approved_refund_amount = p_approved_refund_amount,
