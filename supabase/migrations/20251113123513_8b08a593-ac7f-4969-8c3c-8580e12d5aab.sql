-- Drop and recreate the function with correct booking_status enum values
...
BEGIN
  ...
  WITH monthly_data AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE b.status IN ('confirmed', 'completed', 'cancelled_guest', 'cancelled_host')) AS nights_booked,
      ...
      COALESCE(
        SUM(
          CASE 
            WHEN b.status IN ('cancelled_guest', 'cancelled_host') THEN
