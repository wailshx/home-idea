-- Fix create_dispute_with_support_thread to exclude locked threads
CREATE OR REPLACE FUNCTION public.create_dispute_with_support_thread(p_booking_id uuid, p_category dispute_category, p_subject text, p_description text, p_requested_refund_amount numeric DEFAULT NULL::numeric, p_attachment_urls text[] DEFAULT '{}'::text[])
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Find existing UNLOCKED support thread for THIS USER and this booking
  ...
  -- Create new thread if doesn't exist or previous was locked
