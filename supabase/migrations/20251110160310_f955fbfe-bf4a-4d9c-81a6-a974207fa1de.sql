-- Update get_host_earnings_report to include debt adjustments
-- Drop and recreate to add debt_adjustments column
...
BEGIN
  ...
  debt_by_month AS (
    -- Calculate debt adjustments per listing per month based on when dispute was resolved
    SELECT
      b.listing_id,
      DATE_TRUNC('month', d.resolved_at)::DATE as resolution_month,
      SUM(ABS(p.amount)) as debt_amount
    FROM public.payouts p
    INNER JOIN public.disputes d ON d.id = p.dispute_id
    INNER JOIN public.bookings b ON b.id = p.booking_id
    WHERE 
      p.host_user_id = p_host_user_id
      AND p.status IN ('debit', 'partially_settled', 'settled')
      AND p.amount < 0
      AND d.resolved_at IS NOT NULL
      AND d.resolved_at >= p_start_month
      AND d.resolved_at < p_end_month
    GROUP BY b.listing_id, DATE_TRUNC('month', d.resolved_at)
  ),
  aggregated_report AS (
    SELECT
      ...
      -- Debt adjustments for this month
      COALESCE(dbm.debt_amount, 0) as debt_adjustments
      
    FROM monthly_bookings mb
    LEFT JOIN debt_by_month dbm ON dbm.listing_id = mb.listing_id 
      AND dbm.resolution_month = mb.month_start
    GROUP BY mb.listing_id, mb.listing_title, mb.month_display, mb.month_start, dbm.debt_amount
  )
  SELECT
    ar.listing_id,
    ar.listing_title,
