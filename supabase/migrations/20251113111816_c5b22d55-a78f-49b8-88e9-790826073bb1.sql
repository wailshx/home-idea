-- Fix column references in get_host_earnings_report to use correct table columns
...
BEGIN
  ...
  SELECT * FROM (
    SELECT
      ...
      COALESCE(mb.gross_earnings, 0) AS gross_earnings,
      COALESCE(mb.platform_fees, 0) AS platform_fees,
      COALESCE(mb.net_earnings, 0) AS net_earnings,
      ...
      COALESCE(mb.net_earnings, 0) + COALESCE(pd.cancellation_income, 0) + COALESCE(pd.dispute_income, 0) AS actual_net_earnings
