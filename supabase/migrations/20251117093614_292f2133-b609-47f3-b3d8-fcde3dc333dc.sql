-- Fix admin_suspend_user to use 'blocked' instead of 'inactive' for listings
CREATE OR REPLACE FUNCTION public.admin_suspend_user(p_user_id uuid, p_admin_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Block all user's listings (if they're a host)
  UPDATE listings 
...
-- Fix admin_delete_user_soft to use 'blocked' instead of 'inactive' for listings
CREATE OR REPLACE FUNCTION public.admin_delete_user_soft(p_user_id uuid, p_admin_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
