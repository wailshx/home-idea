-- Drop all versions of host_search_payouts function
DROP FUNCTION IF EXISTS public.host_search_payouts(uuid, text, text, text, numeric, numeric, text, text) CASCADE;

-- Recreate with legacy columns removed (dispute_id, debt_applied_amount)
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      THEN (SELECT category::text FROM disputes WHERE id = p.dispute_ids[1])
      ELSE NULL 
