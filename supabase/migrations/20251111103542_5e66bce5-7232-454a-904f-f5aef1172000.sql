...
-- Drop and recreate host_search_payouts with enhanced financial tracking
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(guest_prof.first_name || ' ' || guest_prof.last_name, 'N/A') as guest_name,
    COALESCE(guest_prof.email, '') as guest_email,
    -- No dispute for regular cancellations
    NULL::uuid as dispute_id,
    -- Enhanced fields for cancelled bookings
    b.status::text as booking_status,
  ...
  ORDER BY
    ...
    CASE WHEN sort_by = 'amount' AND sort_order = 'desc' THEN ABS(amount) END DESC,
    CASE WHEN sort_by = 'amount' AND sort_order = 'asc' THEN ABS(amount) END ASC,
    CASE WHEN sort_by = 'payout_date' AND sort_order = 'desc' THEN payout_date END DESC,
    CASE WHEN sort_by = 'payout_date' AND sort_order = 'asc' THEN payout_date END ASC,
    created_at DESC;
END;
$function$;
