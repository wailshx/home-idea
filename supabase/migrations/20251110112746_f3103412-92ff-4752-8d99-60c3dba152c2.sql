-- 1. Create thread_type enum (skip if exists)
DO $$ BEGIN
...
-- 2. Add thread_type column to message_threads if not exists
DO $$ BEGIN
...
-- 3. Create index for efficient admin support thread queries (skip if exists)
CREATE INDEX IF NOT EXISTS idx_threads_support ON public.message_threads(thread_type, last_message_at DESC) 
WHERE thread_type = 'user_to_support';

-- 4. Create function to get or create a support thread for a user
...
-- 5. Create function for admins to fetch support conversations
...
-- 6. Update search_inbox_conversations to include support threads
...
-- 7. RLS Policies for support threads
