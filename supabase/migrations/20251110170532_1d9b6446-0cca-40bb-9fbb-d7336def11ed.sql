...
-- Fix admin_resolve_dispute to pass thread_id instead of dispute_id to send_dispute_resolution_message
...
BEGIN
  ...
  IF p_is_submit THEN
    ...
    IF p_admin_decision = 'approved' THEN
      ...
      -- Send automated resolution message and lock thread (FIX: use support_thread_id instead of dispute_id)
    ...
    ELSE
      -- Hold status - just update status
      UPDATE public.disputes
      SET
        status = 'in_progress',
        admin_decision = p_admin_decision,
