-- Update get_host_earnings_report to properly attribute dispute refunds
-- This fixes SCENARIO 1C where dispute refunds are deducted from a different listing's payout

-- Drop existing functions
DROP FUNCTION IF EXISTS get_host_earnings_report(UUID, DATE, DATE);
DROP FUNCTION IF EXISTS get_host_earnings_report(UUID);

-- Recreated with fixed dispute_refunds calculation
...
BEGIN
  ...
  payout_data AS (
    SELECT
      ...
      SUM(
        CASE 
          WHEN p.transaction_type = 'refund_debt' THEN ABS(COALESCE(p.total_dispute_refunds, 0))
          WHEN p.transaction_type = 'booking_payout' AND COALESCE(p.total_dispute_refunds, 0) > 0 
