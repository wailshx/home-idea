-- Drop and recreate host_search_payouts with ambiguity-free references
...
BEGIN
  ...
  WITH all_transactions AS (
    ...
    SELECT 
      NULL::uuid as id,
      b.id as booking_id,
      COALESCE(-1 * refund_tx.refund_tx_amount, 0) as amount,
      b.currency,
      'cancelled'::text as status,
      refund_tx.refund_tx_created_at as payout_date,
      b.created_at,
      CASE 
      ...
      refund_tx.refund_tx_amount as refund_amount,
      b.updated_at as cancellation_date
    FROM bookings b
    JOIN listings l ON l.id = b.listing_id
    LEFT JOIN profiles guest_prof ON guest_prof.id = b.guest_user_id
    LEFT JOIN LATERAL (
      SELECT tr.amount AS refund_tx_amount, tr.created_at AS refund_tx_created_at
    ...
    WHERE 
      ...
      AND (min_amount IS NULL OR COALESCE(refund_tx.refund_tx_amount, 0) >= min_amount)
      AND (max_amount IS NULL OR COALESCE(refund_tx.refund_tx_amount, 0) <= max_amount)
