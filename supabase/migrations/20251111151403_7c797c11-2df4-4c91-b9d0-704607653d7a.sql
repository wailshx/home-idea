-- Drop and recreate get_host_earnings_report with corrected dispute refund and debt tracking
...
BEGIN
  ...
  -- Track cancellation refunds by booking's check-in month (negative payouts from cancellations)
  ...
  -- FIXED: Track BOTH negative payouts AND reduced pending payouts (debt_applied_amount with dispute_id)
  dispute_refunds_by_month AS (
    SELECT
      ...
      SUM(
        CASE 
          -- Negative payouts (completed payout refunds) - status would be 'completed' with negative amount
          WHEN p.amount < 0 THEN ABS(p.amount)
          -- Reduced pending payouts (debt_applied_amount tracks the refund) - has dispute_id
          WHEN p.debt_applied_amount > 0 AND p.dispute_id IS NOT NULL THEN p.debt_applied_amount
          ELSE 0
        END
      ) as refund_amount
    FROM public.payouts p
  ...
  -- Track debt deductions by booking's check-in month
  -- FIXED: Exclude dispute refunds (which also use debt_applied_amount but have dispute_id)
  debt_deductions_by_month AS (
    SELECT
      b.listing_id,
      DATE_TRUNC('month', b.checkin_date)::DATE as booking_month,
      SUM(p.debt_applied_amount) as debt_amount
    FROM public.payouts p
    INNER JOIN public.bookings b ON b.id = p.booking_id
    WHERE 
      p.host_user_id = p_host_user_id
      AND p.debt_applied_amount > 0
      AND p.dispute_id IS NULL  -- Exclude dispute refunds to avoid double-counting
      AND b.checkin_date >= p_start_month
      AND b.checkin_date < p_end_month
    GROUP BY b.listing_id, DATE_TRUNC('month', b.checkin_date)
  ),
