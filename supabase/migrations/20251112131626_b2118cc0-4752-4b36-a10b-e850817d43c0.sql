-- Update create_payout_on_booking_completion trigger to use array-based tracking
...
DECLARE
  v_host_user_id UUID;
  v_debt RECORD;
  v_debt_payout_ids UUID[] := ARRAY[]::UUID[];
...
BEGIN
  ...
  -- Collect all refund_debt payouts for this booking
  FOR v_debt IN
    SELECT id, amount, dispute_id
    FROM payouts
    WHERE booking_id = NEW.id
  ...
  LOOP
    v_debt_payout_ids := array_append(v_debt_payout_ids, v_debt.id);
    IF v_debt.dispute_id IS NOT NULL THEN
      v_dispute_ids := array_append(v_dispute_ids, v_debt.dispute_id);
    END IF;
  ...
  -- Create payout record with array-based tracking
  INSERT INTO payouts (
    booking_id,
    host_user_id,
    amount,
  ...
  -- Mark all debt payouts as settled
  IF array_length(v_debt_payout_ids, 1) > 0 THEN
    UPDATE payouts
    SET 
      status = 'settled',
      payout_date = NOW(),
      ...
      updated_at = NOW()
    WHERE id = ANY(v_debt_payout_ids);
  END IF;
