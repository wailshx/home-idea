-- Fix transaction_type constraint to include 'refund_debt' and update process_guest_refund

-- Step 1: Update constraint to include 'refund_debt'
ALTER TABLE public.payouts 
DROP CONSTRAINT IF EXISTS payouts_transaction_type_check;

ALTER TABLE public.payouts
ADD CONSTRAINT payouts_transaction_type_check 
CHECK (transaction_type IN ('booking_payout', 'debt_collection', 'refund', 'cancelled', 'refund_debt'));

-- Step 2: Fix process_guest_refund to set transaction_type when creating debt records
