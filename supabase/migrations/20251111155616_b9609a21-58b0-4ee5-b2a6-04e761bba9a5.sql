-- Fix ambiguous column reference and cancellation refund calculation in get_host_earnings_report
...
BEGIN
  ...
  -- FIXED: Calculate actual host loss from cancellations (what they would get minus what they actually got)
  cancellation_refunds_by_month AS (
    SELECT
      l.id as listing_id,
      DATE_TRUNC('month', b.checkin_date)::DATE as booking_month,
      SUM(
        -- Calculate actual loss: what host would get minus what they actually got
        COALESCE(b.host_payout_net, (b.subtotal + COALESCE(b.cleaning_fee,0)) - COALESCE(b.host_commission_amount,0))
        - COALESCE(
            (SELECT p.amount 
