-- Recreate get_host_earnings_report to fix cancellation logic (host-side only)
...
BEGIN
  ...
  WITH monthly_bookings AS (
    SELECT
      ...
      -- FIXED: Gross earnings include cleaning fee, but ONLY for completed bookings
  ...
  -- FIXED: Host-side cancellation impact = what host would have received if completed
  ...
  -- Dispute refunds (unchanged): reductions on payouts and debit payouts linked to disputes
  ...
  FROM (
    SELECT *,
      CASE 
        WHEN SUM(nights_booked) OVER (PARTITION BY listing_id, month_date) > 0 
        THEN ROUND(gross_earnings / NULLIF(SUM(nights_booked) OVER (PARTITION BY listing_id, month_date),0), 2)
        ELSE 0
