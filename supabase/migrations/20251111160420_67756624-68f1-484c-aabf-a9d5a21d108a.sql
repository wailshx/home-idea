-- Drop and recreate get_host_earnings_report with corrected cancellation income logic
...
AS $$
BEGIN
  -- Verify caller is a host or admin and accessing own data
  ...
  WITH monthly_bookings AS (
    SELECT
      ...
      -- Gross earnings include cleaning fee, but ONLY for completed bookings
      CASE WHEN b.status = 'completed'::booking_status 
        THEN b.subtotal + COALESCE(b.cleaning_fee, 0) 
        ELSE 0 
      END as gross_amount,

      -- Platform fees (commission) ONLY for completed bookings
      CASE WHEN b.status = 'completed'::booking_status THEN COALESCE(b.host_commission_amount, 0) ELSE 0 END as commission_amount,

      -- Net payout (host) ONLY for completed bookings
  ...
  -- Calculate what the host ACTUALLY RECEIVED from cancelled bookings (cancellation fees)
  ...
  -- Dispute refunds: reductions on payouts and debit payouts linked to disputes
  dispute_refunds_by_month AS (
    SELECT
      ...
      SUM(
        CASE 
          WHEN p.status IN ('pending', 'cancelled') AND p.debt_applied_amount > 0 AND p.dispute_id IS NOT NULL THEN p.debt_applied_amount
          WHEN p.status = 'debit' AND p.amount < 0 AND p.dispute_id IS NOT NULL THEN ABS(p.amount)
          ELSE 0
        END
      ) as refund_amount
    FROM public.payouts p
  ...
  -- Debt deductions: only separate negative payouts marked as debt_settlement
  ...
  aggregated_report AS (
    SELECT
      ...
      COALESCE(cibm.income_amount, 0) as cancellation_income,
      COALESCE(drbm.refund_amount, 0) as dispute_refunds
    FROM monthly_bookings mb
    LEFT JOIN cancellation_income_by_month cibm 
      ON cibm.listing_id = mb.listing_id AND cibm.booking_month = mb.month_start
    ...
    GROUP BY mb.listing_id, mb.listing_title, mb.month_display, mb.month_start, 
             cibm.income_amount, drbm.refund_amount, ddbm.debt_amount
  )
  SELECT
    ar.listing_id,
    ar.listing_title,
    ...
    ar.last_payout_date,
    ar.debt_adjustments,
    ar.cancellation_income,
    ar.dispute_refunds,
    ROUND(ar.net_earnings + ar.cancellation_income - ar.dispute_refunds - ar.debt_adjustments, 2) as actual_net_earnings
  FROM (
    SELECT 
      aggregated_report.*,
      CASE 
        WHEN SUM(aggregated_report.nights_booked) OVER (PARTITION BY aggregated_report.listing_id, aggregated_report.month_date) > 0 
        THEN ROUND(aggregated_report.gross_earnings / NULLIF(SUM(aggregated_report.nights_booked) OVER (PARTITION BY aggregated_report.listing_id, aggregated_report.month_date),0), 2)
        ELSE 0
